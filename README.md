...
| POST | `/api/graphs/:gid/refresh/complete` | Executor's run stamp: sets `last_run_at` + `last_run_summary` + `last_run_id`. Only an explicit complete moves the clock — a run that dies mid-checklist leaves the graph due, so the next poll retries. |
